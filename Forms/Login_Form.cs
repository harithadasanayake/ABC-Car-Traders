    ...
    public partial class Login_Form : Form
    {
        ...
        private void Login_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ABCTraders.UsersDataTable tb = new DataSet.ABCTradersTableAdapters.UsersTableAdapter().GetByUsername(username);

                if (tb.Count > 0)
                {
                    // Retrieve the stored hashed password from the database
                    string storedHashPassword = tb[0].Password;

                    // Verify the entered password against the stored hash
                    bool isPasswordValid = BCrypt.Net.BCrypt.Verify(password,storedHashPassword);
